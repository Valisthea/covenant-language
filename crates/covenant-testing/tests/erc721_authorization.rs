...
fn unauthorized_caller_cannot_steal_token() {
    ...
    // Alice mints token #1 to herself (mint is open-access in V0.9).
    h.call_ok(nft, alice, MINT, &[alice.to_u256(), id]);
...
fn transfer_from_wrong_owner_reverts() {
    ...
    h.call_ok(nft, alice, MINT, &[alice.to_u256(), id]);
    // `from` (bob) is not the real owner — reverts NotTokenOwner.
    h.call_revert(
        nft,
        alice,
        TRANSFER_FROM,
